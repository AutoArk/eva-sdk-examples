...
| Client SDK | Flutter | [`autoark_eva_client_sdk`](https://pub.dev/packages/autoark_eva_client_sdk/versions/0.0.1) | [`conversation-agent`](client-sdk/flutter/conversation-agent/) | Flutter mobile 共用 UI；当前消费 pub.dev `0.0.1` 并支持 Android，iOS host 为后续 native implementation 预留 |
...
- Demo 提交的 manifest 与 lockfile 只消费公开发布的 SDK，不通过 workspace、源码相对路径或本地 tarball 回连 SDK 仓库。需要联调本地 SDK 时，只能使用各 demo 明确提供且被 Git 忽略的 override 入口，不得改写或提交公开依赖事实。
- 每个可运行 demo 都应提供只接收文件路径的 key-file 启动入口；demo 通过映射数组声明需要的变量，当前至少包含 `EVA_GATEWAY_API_KEY=<value>`，并转换为自身运行时需要的进程环境或权限受限、退出即删除的临时输入。
...
node --test client-sdk/flutter/conversation-agent/scripts/*.test.mjs
uv run \
  --directory "$(git rev-parse --show-toplevel)/client-sdk/python/voice-dialogue-agent" \
